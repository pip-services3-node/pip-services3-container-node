...
class LinkReferencesDecorator extends ReferencesDecorator_1.ReferencesDecorator {
    ...
    open(correlationId, callback) {
        if (!this._opened) {
            ...
            pip_services3_commons_node_1.Referencer.setReferences(this.topReferences, components);
        }
        if (callback)
            callback(null);
    }
    ...
    close(correlationId, callback) {
        if (this._opened) {
            this._opened = false;
            let components = this.getAll();
            pip_services3_commons_node_1.Referencer.unsetReferences(components);
        }
        if (callback)
            callback(null);
    }
    ...
    put(locator, component) {
        ...
        if (this._opened)
            pip_services3_commons_node_1.Referencer.setReferencesForOne(this.topReferences, component);
    }
    /**
    ...
    remove(locator) {
        ...
        if (this._opened)
            pip_services3_commons_node_1.Referencer.unsetReferencesForOne(component);
        return component;
    }
    /**
